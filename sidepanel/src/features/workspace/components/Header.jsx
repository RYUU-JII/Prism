...
const Header = ({
  ...
  const coreButtonDefs = [
    ...
    {
      id: "overlayToggle",
      title: isViewModeEnabled ? "Show Overlays" : "Hide Overlays",
      tooltip: isViewModeEnabled ? "Show Overlays" : "Hide Overlays",
      action: onToggleViewMode,
      pressed: isViewModeEnabled,
      icon: isViewModeEnabled ? (
        <svg viewBox="0 0 24 24" aria-hidden="true">
        ...
        <div className="panel-shell__settings-content">
          ...
          {settingsTab === "picker" && (
            <div className="panel-shell__settings-section">
              ...
              <div className="panel-shell__setting-row is-stacked">
                <div className="panel-shell__setting-copy">
                  <div className="panel-shell__setting-title">Picker Trigger Key</div>
                  <div className="panel-shell__setting-description">피커 활성화를 위한 수정 키 (Shift, Alt 등)</div>
                </div>
                <div className="panel-shell__segmented">
                  {[
                    ...
                    <button
                      key={key.id}
                      type="button"
                      className={`panel-shell__segment ${uiSettings?.pickerTriggerKey === key.id ? "is-active" : ""}`}
                      onClick={() => onUpdateSetting?.("pickerTriggerKey", key.id)}
                    >
                      {key.label}
                    </button>
                  ))}
